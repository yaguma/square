...
**重要**: すべてのパターンは2x2の4マス全てにブロックが存在します。nullのマスは存在しません。
...
**注意**: Pattern3x1では4マス全てにブロックが配置されます。空きマス(null)は存在しません。
...
**戻り値**: `Block | null` - ブロック、またはx/yが範囲外の場合null

**注意**: 2x2の範囲内(x: 0-1, y: 0-1)であれば、必ずBlockが返されます。nullが返されるのは範囲外の座標が指定された場合のみです
