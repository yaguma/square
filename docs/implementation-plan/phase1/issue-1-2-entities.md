...
**【Phase 1-3で追加実装する範囲】**:
3. 消去可能な矩形を検索（BlockMatchingService）
4. ブロック削除と連鎖処理（BlockRemovalService）
5. 自由落下（BlockFallService）
6. スコア加算
7. 次のブロック生成

**Phase 1-2での実装方針**:
- 基本的なブロック固定処理とゲームオーバー判定のみを実装
- 消去判定・連鎖処理の部分は、TODOコメントで明示
- ドメインサービス実装後（Phase 1-3）に完全な実装を追加
